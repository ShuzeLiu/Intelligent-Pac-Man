...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for (possibleState,probability) in self.mdp.getTransitionStatesAndProbs(state,action):
            qValue += probability*(self.mdp.getReward(state,action,possibleState)+self.getValue(possibleState)*self.discount)
        return qValue
    ...
    def computeActionFromValues(self, state):
        ...
        if self.mdp.isTerminal(state):
            return None
        tempAction =self.mdp.getPossibleActions(state)[0]
        max=self.getQValue(state, tempAction)
