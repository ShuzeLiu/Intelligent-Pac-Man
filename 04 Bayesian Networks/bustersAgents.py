...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # closets ghost' position
        closestGhostPos = None
        closestDistance = 999999
        for pos in ghostPositions:
            if closestDistance > self.distancer.getDistance(pacmanPosition, pos):
                closestGhostPos = pos
                closestDistance = self.distancer.getDistance(pacmanPosition, pos)

        # find the action which can minimize the distance between Pacman and the closest ghost.
        pacAction = None
        minDistance = 999999
        for action in legal:
            successorPosition  = Actions.getSuccessor(pacmanPosition, action)
            afterDistance = self.distancer.getDistance(successorPosition , closestGhostPos)
            if  minDistance > afterDistance:
                pacAction = action
