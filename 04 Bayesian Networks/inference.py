    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPosition in self.legalPositions:
            for newPosition, prob in self.getPositionDistribution(self.setGhostPosition(gameState, oldPosition)).items():
                allPossible[newPosition] = self.beliefs[oldPosition] * prob + allPossible[newPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            ...
            particlePro = self.getBeliefDistribution()
            for particle in particlePro:
                particlePro[particle] = emissionModel[util.manhattanDistance(pacmanPosition, particle)] * particlePro[particle]

            if particlePro.totalCount() > 0: #update
                particlePro.normalize()
                self.particles = []
                for i in range(self.numParticles):
                    self.particles.append(util.sample(particlePro))

            if particlePro.totalCount() == 0: #reset
                self.initializeUniformly(gameState)

        else:
             self.particles = [self.getJailPosition()]   #Can only be in the jail.
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        particlePro = util.Counter()
        for particle in self.particles:
            particlePro[particle] += 1
        particlePro.normalize()
        return particlePro
